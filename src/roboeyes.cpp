...
void loop() {
    ...
    if (lectura != BUTTON.state) {
        ...
        if (lectura == LOW) roboEyes.setMood(HAPPY); // set mood to HAPPY when button is pressed
        else                roboEyes.setMood(DEFAULT); // set mood to DEFAULT when button is released
    }
